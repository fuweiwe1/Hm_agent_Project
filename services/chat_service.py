...
from schemas.app_models import ChatRequest, ChatResponse, ReportRequest
    ...
    def handle(self, request: ChatRequest) -> ChatResponse:
        ...
        if self._is_report_request(request.message):
            report = self.report_workflow.generate_report(
                ReportRequest(user_context=request.user_context, month=explicit_month)
            )
            return ChatResponse(
        ...
        agent = ReactAgent(self.business_service, request.user_context)
